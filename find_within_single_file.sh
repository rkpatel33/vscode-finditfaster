...
# Turn off previews
PREVIEW_ENABLED=${FIND_WITHIN_SINGLE_FILE_PREVIEW_ENABLED:-1}
...
FZF_CMD="${RG_PREFIX+"${RG_PREFIX[@]}"} '$QUERY' $FILE_PATH"
...
IFS=: read -ra VAL < <(
  ...
  fzf --ansi \
      --cycle \
      --bind "change:reload:sleep 0.1; $RG_PREFIX_STR $RG_QUERY_PARSING $FILE_PATH || true" \
      --delimiter : \
